...
mod flags;
pub use flags::{RecordFlags, SignatureScheme};
...
//   152 + tags_padded_len() + payload_padded_len() + padded_sig_len == self.0.len()
...
impl Record {
    ...
    pub unsafe fn from_bytes_unchecked(input: &[u8]) -> Result<&Record, Error> {
        ...
        let unpadded_tags_len = u16::from_le_bytes(input[LEN_T_RANGE].try_into().unwrap()) as usize;
        let unpadded_payload_len =
            u32::from_le_bytes(input[LEN_P_RANGE].try_into().unwrap()) as usize;
        let len = full_len(unpadded_tags_len, unpadded_payload_len, 64);
        if len > 1_048_576 {
    ...
    pub fn write_record<'a>(
        ...
        // Length checks
        let tags_len = parts.tag_set.as_bytes().len();
        if tags_len > u16::MAX as usize {
            return Err(InnerError::RecordTooLong.into());
        }
        let payload_len = parts.payload.len();
        if payload_len > u32::MAX as usize {
            return Err(InnerError::RecordTooLong.into());
        }
        let sig_len = 64;
        let len = full_len(tags_len, payload_len, sig_len);
        if len > 1_048_576 {
            return Err(InnerError::RecordTooLong.into());
        }
        if buffer.len() < len {
        ...
        // Copy in payload
        buffer[payload_range_unpadded(tags_len, payload_len)].copy_from_slice(parts.payload);

        // Copy in tags
        buffer[tags_range_unpadded(tags_len)].copy_from_slice(parts.tag_set.as_bytes());

        // Write LenP
        #[allow(clippy::cast_possible_truncation)]
        buffer[LEN_P_RANGE].copy_from_slice((payload_len as u32).to_le_bytes().as_slice());

        // Write LenS
        #[allow(clippy::cast_possible_truncation)]
        buffer[LEN_S_RANGE].copy_from_slice((64_u16).to_le_bytes().as_slice());

        // Write LenT
        #[allow(clippy::cast_possible_truncation)]
        buffer[LEN_T_RANGE].copy_from_slice((tags_len as u16).to_le_bytes().as_slice());
    ...
    pub fn verify(&self) -> Result<(), Error> {
        ...
        let tags_len = self.tag_set_padded_len();
        let payload_len = self.payload_padded_len();
        let sig_len = 64;
        if HEADER_LEN + tags_len + payload_len + sig_len != self.0.len() {
    ...
    pub fn signature_len(&self) -> usize {
        u16::from_le_bytes(self.0[LEN_S_RANGE].try_into().unwrap()) as usize
    }
    ...
    /// # Panics
    ///
    /// Panics if the signature is not 64 bytes long, which is currently unsupported.
    #[must_use]
    pub fn signature(&self) -> Signature {
        let sig_len = self.signature_len();
        assert!(sig_len == 64, "Unsupported signature not 64 bytes long");
        Signature::from_slice(
            &self.0[sig_range_unpadded(
                self.tag_set_padded_len(),
                self.payload_padded_len(),
                sig_len,
            )],
        )
        .unwrap()
    ...
    pub fn flags(&self) -> RecordFlags {
        RecordFlags::from_bits_retain(u64::from_le_bytes(self.0[FLAGS_RANGE].try_into().unwrap()))
    }
...
const ID_RANGE: Range<usize> = 0..48;
const ID_TIMESTAMP_RANGE: Range<usize> = 0..8;
const ID_HASH_RANGE: Range<usize> = 8..48;

const ADDR_RANGE: Range<usize> = 48..96;
const ADDR_NONCE_RANGE: Range<usize> = 48..56;
const ADDR_KIND_RANGE: Range<usize> = 56..64;
const ADDR_AUTHOR_KEY_RANGE: Range<usize> = 64..96;

const SIGNING_KEY_RANGE: Range<usize> = 96..128;

const TIMESTAMP_RANGE: Range<usize> = 128..136;
const FLAGS_RANGE: Range<usize> = 136..144;
const LEN_T_RANGE: Range<usize> = 144..146;
const LEN_S_RANGE: Range<usize> = 146..148;
const LEN_P_RANGE: Range<usize> = 148..152;

const HEADER_LEN: usize = 152;

fn tags_range_unpadded(tags_len: usize) -> Range<usize> {
    HEADER_LEN..HEADER_LEN + tags_len
}

fn payload_range_unpadded(tags_len: usize, payload_len: usize) -> Range<usize> {
    HEADER_LEN + padded_len!(tags_len)..HEADER_LEN + padded_len!(tags_len) + payload_len
}

fn hashable_range(tags_len: usize, payload_len: usize) -> Range<usize> {
    48..HEADER_LEN + padded_len!(tags_len) + padded_len!(payload_len)
}

fn sig_range_unpadded(tags_len: usize, payload_len: usize, sig_len: usize) -> Range<usize> {
    HEADER_LEN + padded_len!(tags_len) + padded_len!(payload_len)
        ..HEADER_LEN + padded_len!(tags_len) + padded_len!(payload_len) + sig_len
}

fn full_len(tags_len: usize, payload_len: usize, sig_len: usize) -> usize {
    HEADER_LEN + padded_len!(tags_len) + padded_len!(payload_len) + padded_len!(sig_len)
}
...
impl RecordParts<'_> {
    ...
    pub fn record_len(&self) -> usize {
        ...
        let padded_sig_len = 64;
        HEADER_LEN + padded_tag_set_len + padded_payload_len + padded_sig_len
    }
}
